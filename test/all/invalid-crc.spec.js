...
it("Invalid CRC", async () => {
	zip.configure({ chunkSize: 128 });
	const zipReader = new zip.ZipReader(new zip.HttpReader(url, { preventHeadRequest: true }));
	const entries = await zipReader.getEntries();
	let data;
	try {
		data = await entries[0].getData(new zip.BlobWriter(getMimeType(entries[0].filename)), { checkSignature: true });
		data = null;
	} catch (error) {
		if (error.message == zip.ERR_INVALID_SIGNATURE) {
			data = await entries[0].getData(new zip.BlobWriter(getMimeType(entries[0].filename)), { checkSignature: false });
