...
it("Zip64 crypto", async () => {
	...
	let data;
	try {
		data = await entries[0].getData(new zip.BlobWriter(getMimeType(entries[0].filename)));
		data = null;
	} catch (error) {
		if (error.message == zip.ERR_ENCRYPTED) {
			data = await entries[0].getData(new zip.BlobWriter(getMimeType(entries[0].filename)), { password: "password" });
		} else {
			throw error;
		}
	}
